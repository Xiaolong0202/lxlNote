...
**垂直拆分**是将表与表的字段的布局进行拆分，而**水平拆分**是将表进行克隆，进行数据分片处理
...
### ApplicationContext 它实现了BeanFactory,但是在GenericApplicationContext中它持有了一个DefaultListableBeanFactory beanFactory，许多beanFactory的接口实现都是靠它来实现的，使用了组合模式
...
public SpringApplication(ResourceLoader resourceLoader, Class<?>... primarySources) {
    	...
    	//表示传入的配置类，一般也就是启动类的主类
		this.primarySources = new LinkedHashSet<>(Arrays.asList(primarySources));
    	// 去查看依赖中的网络类型 ： webflux  servlet  none 三种状态枚举
		this.webApplicationType = WebApplicationType.deduceFromClasspath();
    	//从spring.factories 文件当中获取对应BootstrapRegistryInitializer的实现类 BootstrapRegistrylnitializer是用来初始化BootstrapRegistry的 (DefaultBootstrapContext)也就是引导类对象
		this.bootstrapRegistryInitializers = new ArrayList<>(
				getSpringFactoriesInstances(BootstrapRegistryInitializer.class));
    	//从spring.factories 文件当中获取对应ApplicationContextInitializer的实现类 ApplicationContextlnitializer是用来初始化ApplicationContext  Spring容器的
		setInitializers((Collection) getSpringFactoriesInstances(ApplicationContextInitializer.class));
   		//从spring.factories 文件当中获取对应ApplicationListener的实现类
...
public ConfigurableApplicationContext run(String... args) {
    	...
    	// 创建引导类对象，相当于一个applicationContext 可以往里面注册对象实例，用于spring容器初始化之前
		DefaultBootstrapContext bootstrapContext = createBootstrapContext();
		ConfigurableApplicationContext context = null;
		configureHeadlessProperty();
    	//去spring.factories中获取 SpringApplicationRunListener 接口 的实现对象,用于spring容器的各种活动，如：starting ready failed enviromentPrepared running
		...
		try {
            ...
            // 准备环境  包含了JVM  properties yaml当中的配置文件中配置的信息
			ConfigurableEnvironment environment = prepareEnvironment(listeners, bootstrapContext, applicationArguments);
			configureIgnoreBeanInfo(environment);
            // 打印banner  也就是 springboot 启动时候的那个图
			Banner printedBanner = printBanner(environment);
            //创建spring 容器
			context = createApplicationContext();
			context.setApplicationStartup(this.applicationStartup);
            
           // 预处理 内容  设置环境，使用ApplicationContextInitializer 来初始化容器  ，初始化完成之后 还会将启动容器关闭 ,将BeanDefinetion注册到spring容器当中
			prepareContext(bootstrapContext, context, environment, listeners, applicationArguments, printedBanner);
			//准备好容器 就开始启动容器 解析、扫描配置类 启动服务器 如TomCat
         	 refreshContext(context);
             // 是一个空方法，我们可以自己去继承 并完成一些扩展
...
[Nacos](https://so.csdn.net/so/search?q=Nacos&spm=1001.2101.3001.7020) 集群默认支持的是CAP原则中的AP原则，但是也可切换为CP原则，切换命令如下：

```swift
curl -X PUT '$NACOS_SERVER:8848/nacos/v1/ns/operator/switches?entry=serverMode&value=CP'
```

同时[微服务](https://so.csdn.net/so/search?q=微服务&spm=1001.2101.3001.7020)的bootstrap.properties 需配置如下选项指明注册为临时/永久实例
...
final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
    ...
    if ((tab = table) == null || (n = tab.length) == 0)
        ...
        n = (tab = resize()).length;
    //通过 & 运算求出该数据（key）的数组下标并判断该下标位置是否有数据 (n-1)&hash = hash % n
...
1. 右移16位是为了让高16位也参与运算((n-1) &hash)，可以更好的均匀散列，减少碰撞，进一步降低hash冲突的几率
2. 异或运算是为了更好保留两组32位二进制数中各自的特征
...
## e.hash & oldCap==0的算法推导

![image-20240222153418140](mdPic/面试题知识点/image-20240222153418140.png)
...
- 为被修饰的对象添加读写屏障，防止指令的重排序
  ...
  写操作屏障，是禁止volatile变量写上方的写操作 重排序到 下方

  读操作屏障，是禁止volatile变量度下方的操作重排序到上方

![image-20231129153015960](mdPic/面试题知识点/image-20231129153015960.png)
...
**内存泄漏**是指在程序中，已经不再需要使用的内存仍然被占用，没有被正确释放或回收的情况。当内存泄漏发生时，系统的可用内存减少，可能导致性能下降、程序运行变得缓慢，甚至在长时间运行后导致程序崩溃。内存泄漏通常是由于程序中的设计错误或者疏忽引起的。

**内存溢出**是指在程序运行过程中，由于申请的内存超过了系统或程序能够提供的可用内存资源，导致程序无法正常执行的现象。
...
2.介绍一下运行时常量池
●class常量池:可以看作是一-张表， 虚拟机指令根据这张常量表找到要执行的类名、
方法名、参数类型、**字面量**、符号引用等信息，他在class文件当中

**符号引用**只是一些符号,包含在字节码文件的常量池中
它主要包括:
在该类中,出现过的各类包,类,接口,字段,方法等元素的全限定名●当类被加载， 它的常量池信息就会放入运行时常量池，并把里面的符号地址变
为真实地址
...
是jvm在堆上面，给现成分配的一块空间，该空间别的线程可以读取，但是不能在该内存空间上创建对象，TLB的作用是防止在高并发下对象创建时候的堆内存争抢过于激烈,TLB主要用于给线程创建小对象
...
jhat  将dump文件转换为html文件

### 打破双亲委派机制

通过Thread.currentThread().setContextClassLoader()方法设置当前线程中的类加载器
...
为类中的变量static分配内存，并分配类基本类型的常量的初始值

**static final 的基本类型 以及字符串常量的赋值**，其他的会设置为默认值  其他的会在初始化阶段完成
...
指一个对象的作用域只在一个方法当中，就被不称为方法逃逸，反之为方法逃逸，比如，一个对象被当作返回值返回出去了，或者说他本来就是被当作一个参数被传进来的
...
扫描堆中得到对象，从GcRoot对象为起点的引用链开始看能否找到该对像，能被找到则表示可以回收，要全局分析，所以时间过长，内存消耗大
...
- 类静态属性引用的对象
- 静态常量对象
- 本地方法栈中JNI引用的对象，native方法：在本地方法栈中被引用的对象是指在本地方法中被引用或使用的Java对象。当Java程序调用本地方法时，可能需要传递Java对象给本地方法进行处理，或者本地方法可能会创建新的Java对象。这些涉及到的Java对象就存在于本地方法栈中。
- 存活的线程对象
- 被synchornized 锁住的对象
- Class 对象
...
- 当在年轻代的一个对象躲过了 15 次 （次数可以更改）GC ， 就可以晋升至老年代
- 动态年龄分析 ：年轻代中的小于某个年龄的对象的内存之和 占了 年轻代中的内存的百分之五十以上， 那大于这个年龄的所有对象都可以晋升至老年代,可以理解为年轻代中的内存过多了，需要为新来的让出空间，所以就将某些年长一些的对象提前晋升了
- 大对象：  当一个对象过于大的时候 就可以直接晋升为老年代

### 空间分配担保机制

当 yongGC的时候确保老年代能够接收 年轻代中转移过来的数据

显示判断 **老年代中的最大的连续内存空间** 是否大于新生代中的所有对象的总空间

如果成立就进行YoungGC 如果不成立就会进一步**老年代最大连续内存分配空间** 是否大于前几次YoungGC 晋升到老年代的平均内存大小  如果是的话就进行youngGc 

如果不是或者因为老年代内存空间不足导致失败了的话，就会执行fullGC
...
- 空间分配担保机制 失败
- System.gc() 手动触发
- 如果有永久代的话 永久代空间满了 也会触发

## STW

jvm中所有的线程都停止下来，native方法可以执行但是不能与JVM产生交互

主要是垃圾回收导致的

他是不可避免的是为了避免，多标（本不该回收的对象被回收），漏标（该回收的对象没有被回收），清除干净 运行的过程中一直在产生垃圾
...
产生多标产生的条件： 白色节点与一个灰色节点断开接到一个黑色节点上面,才会产生多标。 （如果只是将引用修改到白色节点上面，因为还没有扫描到这个白色节点，所以不可能产生多标）

- **增量更新**  当白色节点与黑色节点连接的时候，将这个黑色节点记录下来，在重新标记的阶段，以这个黑色节点为根节点再开始进行遍历  (CMS)
- **原始快照**  当白色节点与灰色节点断开的时候，将这个快照记录下来，在重新标记的阶段，一灰色节点为根节点标记的时候就会标记到这个结点了，但是可能断开的这个结点会变成垃圾，也有可能是改变了引用，所以可能会产生浮动垃圾  (G1)
...
- **G1**垃圾回收器 JDK9极其以后使用  **整堆收集器**  （**JDK11推出的ZGC也是整堆收集器**）  06 年发布的

将堆内存划分为了多个小的区域，每个对象都可以充当eden 、survivor、old、humongous（存放大对象）

并发手收集，采用多核cpu的优势

G1回收器中的新生代与老年代**不在是物理上的隔离了**，他们都是由一个一个的Region组成的

支持**热插拔**，因为堆是有一个个的Region组成所以可以去，动态的分配堆的大小

**支持可预测停顿** 因为G1是通过内存分块来进行管理的，所以说有一定的预测性,建立可预测的停顿时间模型
...
 在HashMap的put方法里面，是通过key的hash值与数组的长度取模计算得到的一个数组位置。而在绝大部分情况下，n的值一般是小于2^16(就是65536)，这就意味着 i 的值始终是使用hash值的低16位与(n - 1)进行取模计算，这是由 & 运算符的特点决定的，这样就会造成key的散列度不是很高，导致大量的key集中存储在一个固定的几个数组位置上，很显然这会影响到数据的查找性能。因此为了提升key的hash值的一个散列度，在hash方法里面做了一个位移运算。

  所以在hash方法里面，首先使用key的hashCode无符号右移16位，意味着把hashCode的高位移动到了低位，然后再用hashCode与右移之后的值进行异或运算。就相当于把高位和低位的特征进行了组合，这样通过高位和低位组合后的hashCode通过 & 运算符进行运算后，它得到的一个数组的位置的散列度一定会更高，通过这种方式，可以去降低hash冲突的概率。

## ConcurrentHashMap扩容

每个线程根据cpu核心数分配要进行迁移的任务数

根据任务数去领取任务

如果是第一个进来的线程会创建一个新的数组

在迁移的过程中会将oldTable中的Node换成FowardNode,它的hash为-1,这样就可以告诉其他线程现在正在扩容

如果一个线程扩容完成了刚刚的任务，如果还有能够扩容的任务，就会回过头去继续领任务

最后一个扩容的线程会检查确认一遍，迁移是否完毕

## Fail-fast与Fail-safe

在java当中当迭代器迭代的时候的底层的集合被修改时候，迭代器会有两种表现。

fail-fast 是 当集合的修改不符合预期的时候就会抛出CocurrentModifacationException 异常 如当多个线程修改一个集合的时候他是由两个变量：excepteCount  ModCount实现的当两个变量的值不相等的时候就会报错

fail-safe  迭代器进行迭代的时候不会抛出异常 ，即使迭代器维护的集合在迭代的时候发生更改也不会影响运行，一般是通过快照的方式实现、对集合的复制实现的
