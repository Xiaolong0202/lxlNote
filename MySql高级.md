...
其典型的使用场景是做全库的逻辑备份，对所有的表进行锁定

## 表级别锁

### 表锁

- **表共享锁  read lock**  加了表共享锁，他自己也不能进行 改 、删、写
...
是由数据库自动控制的，在表上有活动事务的时候，不可以对元数据进行写入操作
...
防止行锁与表锁的冲突，如果一个表中的行添加了行锁，则不能添加对应表锁，但是肯定不能去每一行去查看有没有添加行锁，所以要给这个表添加一个意向锁，用来判断该表当中是否有无添加行锁
...
### 行锁 / 记录锁    （lock_rec_not_gap）

如果进行操作的时候，涉及到的条件查找操作不通过索引来查找，会将整个表都锁住，是将这一列的**所有行**，以及**所有的间隙**都锁住

通过索引来实现的，MYISAM  存储引擎当中并不支持，所以单个记录，在RR （可重复读）与RC（读已提交）

- 共享锁  LOCK IN SHARE MODE
- 排他锁  FOR UPADTE  如果是DML语句会自动的加上

![image-20231126011745416](mdPic/MySql锁/image-20231126011745416.png)

### 间隙锁(**Gap locks**)  、临键锁 （Next-key locks）行锁+间隙锁

锁定索引的间隙，保证索引记录的间隙不变,解决幻读的问题

**间隙锁可能会产生死锁的问题**，当两个事务都在同一个区间添加了间隙锁，并且都想要对这个区间进行insert操作的时候就会触发死锁问题，因为两个事务都会等待对方事务的资源释放，触发超时，自动回滚某一个事务

- **唯一索引**为**等值查询**的时候，给**不存在的记录**加锁

  比如本来有 id = 3 , id =8

  操纵数据的时候条件 where id = 5,就会给 (3,8)这个区间加锁,不允许对该区间进行操作

- 普通索引 等值查询  会将 索引值为中心，向左第一个比他小的，像右第一个比他大的加锁  因为是非唯一索引，如果**有等值的数插入的话就会出现幻读的问题**
...
- 模糊查询的时候使用了 "%***" %开头的会失效，头部模糊匹配
- 对索引列进行了运算 函数运算
- 索引列出现了类型转换  例如整数与字符串
- 联合索引没遵循最左前缀匹配法则
- 在 WHERE 子句中，如果在 OR 前的条件列是索引列，而在 OR 后的条件列不是索引列，那么索引会失效。得两个都加上索引才行

## 执行计划：

![image-20240305113255592](mdPic/MySql高级/image-20240305113255592.png)
...
![image-20240305113318821](mdPic/MySql高级/image-20240305113318821.png)

### index 与 all 的区别

​	在与全索引扫描的时候不需要查询数据区data域，只要查询索引区域就行了

![image-20240305113955214](mdPic/MySql高级/image-20240305113955214.png)
...
alter table user add index age_index(email(5)); 给字符串的前五个字段添加索引，添加前缀索引，因为如果整个都添加索引的话，可能会造成索引太多
...
- 进行批量插入，也不要太多（不要超过一千条），太多的话可以多次分批插入
- 多次插入的时候 **手动控制事务**，防止频繁的事务提交 
- 主键进行顺序插入 ，和索引有关
- 大量数据用load指令
...
- 主键乱序插入的话容易造成**页分裂**  ,页合并是当页中空闲的空间超过了Threshold阈值就会合并页
- 尽量降低主键的长度
- 不要使用UUID 或者 身份证号这种乱序的作为主键
- 业务操作尽量避免对主键的修改

## OrderBy优化：

- **file sort**  通过扫描全表，并且将所有的行放入 sort_buffer 缓冲区再进行排序操作再进行返回
- **Using index**  通过索引顺序扫描   ，直接返回索引数据
...
先通过主键索引查找分页的主键id(因为主键是顺序的)，然后再去通过主键查出数据，当然是必须要满足主键是按照顺序排列的,或者按照别的索引查询出id
```

## count的优化：

如果是MyisAm 引擎的时候那就是直接返回行数

count (*)  做了优化 **不取值**直接进行累加

count (主键) (**先取值** 然后  直接+1不会判断是否是null,因为不可能为null)、

count (字段)  **先取值**   如果有not null 约束的话就不需要判断是否为 null 如果没有则**会判断**
...
记得加索引，因为没有索引的时候是加的表锁，加了索引的话就会使用行锁了
...
![image-20240304220225973](mdPic/MySql高级/image-20240304220225973.png)

## COMPACT行格式（Mysql 5.1之后，Mysql 5.7之后用的Dynamic）：

![image-20240304220320430](mdPic/MySql高级/image-20240304220320430.png)

**MySQL 规定除了 TEXT、BLOBs 这种大对象类型之外，其他所有的列+变长字段长度列表与NULL之列表（不包括隐藏列和记录头信息）占用的字节长度加起来不能超过 65535 个字节**。

## 变长字段长度列表:

​	记录那些**变长属性**的占用的**字节数**：如varchar ,TEXT,并且是**倒序**的，为了**使得位置靠前的记录的真实数据和数据对应的字段长度信息可以同时在一个 CPU Cache Line 中，这样就可以提高 CPU Cache 的命中率**。  

> 为null的变长字段不会记录长度

## NULL值列表:

​	记录每个列的值**是否为Null** ，一个位表示一个列，为1就是null,并且大小必须是**字节的整数倍**，也是**逆序排放**

> 如果是加了 **not null 约束**的字段就不会对该字段进行记录

## 记录头信息：

delete_mask ：标识此条数据是否被删除、record_type：表示当前记录的类型

## 行溢出

MySQL 中磁盘和内存交互的基本单位是页，一个页的大小一般是 `16KB`，也就是 `16384字节`，而一个 varchar(n) 类型的列最多可以存储 `65532字节`，一些大对象如 TEXT、BLOB 可能存储更多的数据，这时一个页可能就存不了一条记录。这个时候就会**发生行溢出，多的数据就会存到另外的「溢出页」中**。

![image-20240304221500307](mdPic/MySql高级/image-20240304221500307.png)